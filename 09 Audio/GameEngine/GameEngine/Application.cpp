...
bool Application::Initialize (LPCTSTR name, unsigned int width, unsigned int height, bool isFullScreen)
{
	...
	// Shader를 초기화한다.
	if (g_shader.Initialize (L"../../../Shaders/PBRShader.hlsl") == false)
	{
		return false;
	}
	...
	// Audio를 초기화한다.
	if (Audio::GetInstance ().Initialize (256) == false)
	{
		return false;
	}
	std::ifstream audioFile ("../../../Assets/Music.wav", std::ifstream::binary);

	if (audioFile)
	{
		audioFile.seekg (0, std::ios::end);
		fileSize = static_cast<size_t> (audioFile.tellg ());
		audioFile.seekg (0, std::ios::beg);

		buffer = std::make_unique<char[]> (fileSize);
		audioFile.read (buffer.get (), fileSize);
	}
	else
	{
		return false;
	...
	if (WavImporter::Import (g_audioClip, buffer.get (), fileSize) == false)
	{
		return false;
	}

	Audio::GetInstance ().PlayClip (g_audioClip);
...
void Application::Update ()
{
	...
	while (true)
	{
		...
		renderBuffer.CameraPosition = pos;
		renderBuffer.LightColor = g_input.GetKey (KeyCode::Alpha1) ? Vector3 (0.0f, 0.8f, 0.0f) : Vector3 (1.0f, 1.0f, 1.0f);
		renderBuffer.LightDirection = Vector3 (1.0f, -1.0f, 1.0f);
		g_d3d11.UpdateRenderBuffer (renderBuffer);

		Transform& transform = *g_gameObject->GetComponent <Transform> ();
		Vector3 rotation = transform.GetRotation ().Euler ();
		rotation.m_y += 30.0f * g_timer.GetDeltaTime ();
		transform.SetRotation (Quaternion::FromEuler (rotation));

		g_gameObject->GetComponent<Renderer> ()->SetRoughness (g_input.GetKey (KeyCode::Alpha2) ? 1.0f : 0.04f);
		g_gameObject->GetComponent<Renderer> ()->SetMetallic (g_input.GetKey (KeyCode::Alpha3) ? 1.0f : 0.0f);
